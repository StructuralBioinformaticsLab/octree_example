...
#include _MOL_INCLUDE_
  ...
  #define DEFAULT_VDW_DIST_CUTOFF 9.0
#endif
...
int PAPI_events[ NUM_PAPI_EVENTS ] = { PAPI_L1_DCM, PAPI_L2_DCM, PAPI_L3_TCM };//, PAPI_L1_DCM, PAPI_L2_DCM, PAPI_L3_DCM, PAPI_TOT_CYC, PAPI_TOT_INS };
//int PAPI_events[ NUM_PAPI_EVENTS ] = { PAPI_L1_DCM, PAPI_TOT_CYC, PAPI_TOT_INS };//, PAPI_L1_DCM, PAPI_L2_DCM, PAPI_L3_DCM, PAPI_TOT_CYC, PAPI_TOT_INS };
...
struct my_par
{
   ...
   unsigned long long updateTime, energyTime, computeTime;
   long numUpdates;
...
void my_en_grad( int n, double *inp, void *prms, double *en, double *grad )
{
    ...
    if ( octpar != NULL )
      {
        prm->numUpdates++; 
        reorganize_octree( octpar->octree_static, 1 );
      }  
    else
      {
        if ( mqcheck )
          {
            prm->numUpdates++;           
            check_clusterupdate( mag, mags );
          }  
      }

    end = get_time( );
    
    prm->updateTime += ( end - start );
    ...
    *en = 0;
    ...
    unsigned long long start_e, end_e;
    ...
    if ( octpar == NULL )
      ...
      {
        linearize_atoms_inside_octree_nodes( 0, 0, octpar->octree_static );
...
//#ifdef VECTORIZE_OCTREE                                                     
//        copy_force_from_octree_to_atoms( 0, octpar->octree_static );
//#endif                                                             
...
void papi_test( void )
{
    ...
    if ( ( i = PAPI_num_counters( ) ) <= PAPI_OK )  
      {
        print_error( "Failed to initialize PAPI!" );
        exit( 1 );
      }  
    ...
    for ( i = 0; i < NUM_PAPI_EVENTS; i++ )
      {
        if ( PAPI_query_event( PAPI_events[ i ] ) != PAPI_OK )
          {
            printf( "No counter for event %d ( %d )!\n", i, PAPI_events[ i ] );
            exit( 1 );
          }  
      }  
}
...
{
   long long PAPI_values_nblist[ NUM_PAPI_EVENTS ];
   long long PAPI_values_octree[ NUM_PAPI_EVENTS ];   
   ...
   charmmTime = ( ( double ) ( end - start ) ) / 1000.f;
   printf( "%.3lf ms\n", charmmTime );
       ...
       if ( !noNblist )
         {
           ...
           nblist_cons = ( ( double ) ( end - start ) ) / 1000.f;
           printf( "%.3lf ms\n", nblist_cons );
...
//           if ( !build_octree( &octree, 60, 6, 1.0, ag ) )
           if ( !build_octree( &octree, 150, 10, 1.0, ag ) )
             {
               print_error( "Failed to build octree!" );
               return 1;
             }
           end = get_time( );
           octree_cons = ( ( double ) ( end - start ) ) / 1000.f;
           printf( "%.3lf ms\n", octree_cons );
       ...
       double nblist_init_E = 0, octree_init_E = 0, nblist_E = 0, octree_E = 0, nblist_min = 0, octree_min = 0, nblist_utime = 0, octree_utime = 0, nblist_etime = 0, octree_etime = 0, nblist_ctime = 0, octree_ctime = 0;
       long nblist_nupdates = 0, octree_nupdates = 0;
       const char *min_method[ ] = { "LBFGS" };
    
       if ( !noNblist )
         {
           mupa.octpar = NULL;
           mupa.updateTime = mupa.energyTime = mupa.computeTime = 0;
           mupa.numUpdates = 0;
           ...
           PAPI_stop_counters( PAPI_values_nblist, NUM_PAPI_EVENTS );                      
           nblist_min = ( ( double ) ( end - start ) ) / 1000.f;
           nblist_utime  = ( ( double ) mupa.updateTime ) / 1000.f;
           nblist_etime  = ( ( double ) mupa.energyTime ) / 1000.f;           
           nblist_ctime  = ( ( double ) mupa.computeTime ) / 1000.f;
           nblist_nupdates = mupa.numUpdates;
           my_en_grad( 0, NULL, ( void * )( &mupa ), &nblist_E, NULL );
           printf("\ndone ( time = %.3f ms, update time = %.3f ms ( %.3f ms ), \#updates = %ld, final energy = %lf kcal/mol )\n\n", nblist_min, nblist_utime, nblist_min - nblist_ctime, nblist_nupdates, nblist_E );
       ...
       if ( !noOct )
         {
           ...
           PAPI_stop_counters( PAPI_values_octree, NUM_PAPI_EVENTS );           
           octree_min = ( ( double ) ( end - start ) ) / 1000.f;
           octree_utime  = ( ( double ) mupa.updateTime ) / 1000.f;
           octree_etime  = ( ( double ) mupa.energyTime ) / 1000.f;           
           octree_ctime  = ( ( double ) mupa.computeTime ) / 1000.f;
           octree_nupdates = mupa.numUpdates;
           my_en_grad( 0, NULL, ( void * )( &mupa ), &octree_E, NULL );
           printf("\ndone ( time = %.3f ms, update time = %.3f ms ( %.3f ms ), \#updates = %ld, final energy = %f kcal/mol )\n\n", octree_min, octree_utime, octree_min - octree_ctime, octree_nupdates, octree_E );
       ...
       if ( !noNblist ) 
         {  
           ...
           printf( "\tconstruction time = %.3lf ms\n", nblist_cons );
           printf( "\trunning time = %.3lf ms\n", nblist_min );       
           printf( "\tupdate time = %.3lf ms ( %.3lf ms )\n", nblist_utime, nblist_min - nblist_ctime );       
           printf( "\tenergy time = %.3lf ms\n", nblist_etime );       
           ...
           printf( "\tcache misses < L1, L2, L3 > = < %lld, %lld, %lld >\n", PAPI_values_nblist[ 0 ], PAPI_values_nblist[ 1 ], PAPI_values_nblist[ 2 ] );         
       ...
       if ( !noOct ) 
         {
           ...
           printf( "\tconstruction time = %.3lf ms\n", octree_cons );
           printf( "\trunning time = %.3lf ms\n", octree_min );                   
           printf( "\tupdate time = %.3lf ms ( %.3lf ms )\n", octree_utime, octree_min - octree_ctime );       
           printf( "\tenergy time = %.3lf ms\n", octree_etime );       
           printf( "\tnumber of updates = %ld\n", octree_nupdates );       
           printf( "\tsize = %d bytes ( %.2lf KB, %.2lf MB )\n",
                    soctree, soctree / 1024.0, soctree / ( 1024.0 * 1024.0 ) );
           printf( "\tcache misses < L1, L2, L3 > = < %lld, %lld, %lld >\n", PAPI_values_octree[ 0 ], PAPI_values_octree[ 1 ], PAPI_values_octree[ 2 ] );                             
           printf( "\tinitial energy = %lf kcal/mol\n", octree_init_E );                                         
           printf( "\tfinal energy = %lf kcal/mol\n", octree_E );                         
           printf( "\n" );                                    
         }           
       ...
       if ( !noNblist && !noOct ) 
         {
           ...
           if ( octree_utime > 0 ) printf("\tupdate-time( nblist ) / update-time( octree ) = %.2lf ( %.2lf )\n", nblist_utime / octree_utime, ( nblist_min - nblist_ctime ) / ( octree_min - octree_ctime ) );
           else printf("\tupdate-time( nblist ) / update-time( octree ) = inf\n" );

           if ( octree_etime > 0 ) printf("\tenergy-time( nblist ) / energy-time( octree ) = %.2lf\n", nblist_etime / octree_etime );
           else printf("\tenergy-time( nblist ) / energy-time( octree ) = inf\n" );

           if ( soctree > 0 ) printf( "\tsize( nblist ) / size( octree ) = %.2lf\n", ( 1.0 * snblist ) / soctree );
           else printf( "\tsize( nblist ) / size( octree ) = inf\n" );       
           
           if ( PAPI_values_octree[ 0 ] > 0 ) printf( "\tL1-misses( nblist ) / L1-misses( octree ) = %.2lf\n", PAPI_values_nblist[ 0 ] / ( 1.0 * PAPI_values_octree[ 0 ] ) );
           else printf( "\tL1-misses( nblist ) / L1-misses( octree ) = inf\n" );

           if ( PAPI_values_octree[ 1 ] > 0 ) printf( "\tL2-misses( nblist ) / L2-misses( octree ) = %.2lf\n", PAPI_values_nblist[ 1 ] / ( 1.0 * PAPI_values_octree[ 1 ] ) );
           else printf( "\tL2-misses( nblist ) / L2-misses( octree ) = inf\n" );

           if ( PAPI_values_octree[ 2 ] > 0 ) printf( "\tL3-misses( nblist ) / L3-misses( octree ) = %.2lf\n", PAPI_values_nblist[ 2 ] / ( 1.0 * PAPI_values_octree[ 2 ] ) );
           else printf( "\tL3-misses( nblist ) / L3-misses( octree ) = inf\n" );
